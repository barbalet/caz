...
`programs/survival.inc` is the shared survival routine included by part of the archive. It requests `NAV_CHARGER` when slots are available, `NAV_SOLAR` when conservation is the safer choice, and `NAV_JUNCTION` when a nearby junction can be tapped. Standalone-energy programs such as `feral-forager.caz`, `long-room-prowl.caz`, `sunward-forager.caz`, and `perimeter-stalker.caz` keep those recovery choices inside their own behavior flow and are tested not to include the shared file. The environment can still enforce last-ditch safety, but a successful long-run experiment should not require that fallback.
